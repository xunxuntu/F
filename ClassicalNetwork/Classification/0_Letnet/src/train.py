...
for epoch in range(5):  # 一个epoch即对整个训练集进行一次训练
    ...
    for step, data in enumerate(train_loader, start=0):  # 遍历训练集，step从0开始计算
        ...
        optimizer.zero_grad()  # 清除历史损失梯度 ,原因：https://www.zhihu.com/question/303070254
        ...
        if step % 500 == 499:  # print every 500 mini-batches，每500步打印一次
            with torch.no_grad():  # 在以下步骤中（验证过程中）不用计算每个节点的误差损失梯度，防止内存占用，资源消耗
                outputs = net(test_image.to(device))  # 测试集传入网络（test_batch_size=10000），output维度为[batch, 10]这里是[10000,10]
